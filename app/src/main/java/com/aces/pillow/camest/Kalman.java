...
public class Kalman extends KalmanFilter {
    ...
    public Point getPrediction() {
        ...
        est = new Point(prediction.get(0, 0)[0], prediction.get(1, 0)[0]);
        return est;
    }
    ...
    public Point update(Point p, boolean dataCorrect) {
        ...
        if (!dataCorrect) {
            measurement.put(0, 0, est.x);
            measurement.put(1, 0, est.y);
        } else {
        ...
        est.x = estimated.get(0, 0)[0];
        est.y = estimated.get(1, 0)[0];
        return est;
    }
